...
// Check the CC3 permissions
function jak_cc3_access($table, $opid, $id) {
    global $jakdb;
    if ($jakdb->has($table, ["AND" => ["id" => $id, "opid" => $opid]])) {
        return true;
    } else {
        return false;
    }
...
function jak_get_data($id, $opid, $table) { 
    global $jakdb;
...
// Check if row exist
function jak_row_exist($id, $opid, $table, $mainop) {
    global $jakdb;

    if ($mainop && $table == "user" && $jakdb->has($table, ["id" => $id])) {
        return true;
    } elseif ($jakdb->has($table, ["AND" => ["id" => $id, "opid" => $opid]])) {
        return true;
    } else {
        return false;
    }
...
// create session id
function create_session_id($opcacheid, $depid, $opid, $ipa) {
    ...
    if (isset($_SESSION['rlbid']) && !empty($_SESSION['rlbid'])) {
        $jakdb->update("buttonstats", ["depid" => $depid, "opid" => $opcacheid, "singleopid" => $opid, "hits[+]" => 1, "referrer" => $referrer, "ip" => $ipa, "lasttime" => $jakdb->raw("NOW()")], ["session" => $_SESSION['rlbid']]);
        ...
        $jakdb->insert("buttonstats", ["depid" => $depid, "opid" => $opcacheid, "singleopid" => $opid, "referrer" => $referrer, "firstreferrer" => $referrer, "agent" => $clientsystem, "hits" => 1, "ip" => $ipa, "country" => $country_name, "countrycode" => $country_code, "latitude" => $country_lat, "longitude" => $country_lng, "session" => $rlbid, "time" => $jakdb->raw("NOW()"), "lasttime" => $jakdb->raw("NOW()")]);
...
// function online_operators($opid, $dp, $did = 0, $oid = 0) {
...
//     $jakdb->update("user", ["available" => 0], ["AND" => ["OR" => ["id" => $opid, "opid" => $opid], "lastactivity[<]" => $timeout]]);
...
//     $sth = $jakdb->pdo->prepare("SELECT id, hours_array, phonenumber, available, alwaysonline, departments, emailnot, pusho_tok, push_notifications FROM ".JAKDB_PREFIX."user WHERE (id = :opid OR opid = :opid) AND access = 1".$sql_where);

//     if ($opid > 0) $sth->bindParam(':opid', $opid, PDO::PARAM_INT);
...
//             // We go for no operator is online
//             $oponline = false;
...
function online_operator_list_whatsapp($opid, $dp, $did = 0, $oid = 0) {
        ...
        $sth = $jakdb->pdo->prepare("SELECT id, departments, name, picture, aboutme, hours_array, whatsappnumber, available, alwaysonline, alwaysnot FROM ".JAKDB_PREFIX."user WHERE (id = :opid OR opid = :opid) AND access = 1 AND whatsappnumber != ''".$sql_where." ORDER BY departments ASC");
...
function online_operator_list($opid, $dp, $did = 0, $oid = 0) {
    ...
    $sth = $jakdb->pdo->prepare("SELECT id, name, picture, hours_array, aboutme, phonenumber, available, departments, emailnot, pusho_tok, push_notifications, alwaysonline FROM ".JAKDB_PREFIX."user WHERE (id = :opid OR opid = :opid) AND access = 1".$sql_where." ORDER BY departments ASC");
...
function jak_create_htaccess($path) {
...
function jak_load_external_file($path, $orig_name, $mime_type) {
    ...
    // Get the file
    $thefile = str_replace('//','/', CLIENT_UPLOAD_DIR.'/'.$path);
...
function jak_encrypt_decrypt($data, $ed = true) {
    ...
    $key = hash('sha256', JAK_FILE_SECRET_KEY);
    $iv = substr(hash('sha256', JAK_FILE_SECRET_IV), 0, 16);
